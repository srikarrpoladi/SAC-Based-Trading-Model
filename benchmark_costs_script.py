...
class Benchmark:
    ...
    def get_vwap_trades(self, data, initial_inventory, preferred_timeframe=390):
        ...
        """
        total_volume = data['market_volume'].sum()
        total_steps = len(data)
        remaining_inventory = initial_inventory
        trades = []
        for step in range(min(total_steps, preferred_timeframe)):
            volume_at_step = data['market_volume'].iloc[step]
    ...
    def calculate_vwap(self, idx, shares):
        ...
        """
        # Ensure bid_prices and bid_sizes are extracted as arrays for element-wise operations
        bid_prices = np.array([self.data[f'bid_price_{i}'].iloc[idx] for i in range(1, 6)])
        bid_sizes = np.array([self.data[f'bid_size_{i}'].iloc[idx] for i in range(1, 6)])
    
        cumsum = 0
        for i, size in enumerate(bid_sizes):
            size = size.item()  # Convert the Series to scalar using .item()
            cumsum += size  # Now cumsum remains scalar
            if cumsum >= shares:  # Comparison works correctly since cumsum is scalar
                break
    
    # Calculate VWAP as the weighted average of bid prices and bid sizes up to the current index
